...
exports = module.exports = function(req, res) {
	...
	view.on('init', function (next) {
		keystone.list('User').model.find().exec(function (err, feedback) {
			if(feedback == null) res.redirect('/audiences');
			if(req.user){
				console.log("LOLOLOL");
				// console.log(req.user);
				locals.data.currentUser = req.user;
				console.log(locals.data.currentUser);
				if(locals.data.currentUser.description){
					locals.data.currentUser.description = locals.data.currentUser.description.replace(/(<([^>]+)>)/ig,"");
					console.log("WORKING!!!");
					console.log(locals.data.currentUser.description);
				}
