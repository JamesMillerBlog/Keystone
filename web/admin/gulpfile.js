...
gulp.task('default', function(callback){
	runSequence(['sitestyles', 'sass', 'jslibs', 'es6', 'server', 'models', 'updates', 'routes', 'html', 'favico', 'images', 'fonts', 'browserSync', 'watch'], callback )
});
...
gulp.task('watch', function(){
	...
	gulp.watch('app/public/admin/js/*.js', ['es6']); // bundle.js
	gulp.watch('app/public/admin/js/modules/**/*.js', ['es6']); // front end js
	gulp.watch('app/public/admin/js/libs/*.js', ['jslibs']); // front end libs
	gulp.watch('app/public/admin/customStyles/scss/**/*.scss', ['sass']); //sass
});

//task to put all admin files into dist folder
gulp.task('favico', function(){
	return gulp.src('app/public/admin/*.ico')
	.pipe(gulp.dest('dist/public/admin'))
});

//task to optimise images + put them in dist folder
gulp.task('images', function(){
	return gulp.src('app/public/admin/images/**/*.+(png|jpg|gif|svg|mp4|ogv|ogg)')
	.pipe(cache(imagemin({
		interlaced: true
	})))
	.pipe(gulp.dest('dist/public/admin/images/'))
});

gulp.task('fonts', function(){
	return gulp.src('app/public/admin/fonts/**/*')
...
gulp.task('es6', function() { //transform all code into es2015 format
	browserify('app/public/admin/js/bundle.min.js') //take all code from index.js
...
gulp.task('jslibs', function(){
	return gulp.src('app/public/admin/js/libs/*.js')
	.pipe(concat('libs.min.js'))
	.pipe(uglify()) //minifies code
	.pipe(gulp.dest('dist/public/admin/js/libs/'));
});

//task to turn sass into css and then reload browser
gulp.task('sitestyles', function(){
	return gulp.src('app/public/admin/styles/**/*.+(css|scss)')
    .pipe(gulp.dest('dist/public/admin/styles'))
});

//task to turn sass into css and then reload browser
gulp.task('sass', function(){
	return gulp.src('app/public/admin/customStyles/scss/**/*.scss')
	.pipe(sass())
	.pipe(concatCss('styles.min.css'))
    .pipe(gulp.dest('dist/public/admin/customStyles/css/'))
    .pipe(browserSync.reload({
		stream: true
	}))
});
